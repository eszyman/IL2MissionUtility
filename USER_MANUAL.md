...
- **Place** — formation (Inverted Vee is finger-four, also Vee, Pairs, Echelon, Line abreast, Column) and **how many per group**. Spacing is always 150 m. If a unit is a **Lead**, **In formation** sets `NumberInFormation` for that flight (0 = lead) as a modifier on the per-group count. The diagram is the same style as before, now with map-like **Zoom** (slider, scroll wheel, **Reset View**) and right-drag pan. **Zone IN / Zone Out** rings are drawn in metres (zoom out to see them). Waypoint diamonds sit north of the origin; with a **Goto WP** order selected, click a diamond to pick that WP.
...
- `ENABLE / PULSE IN` — Mission Begin and Army Generator fire this; it pulses **Zone IN**.
- `Zone IN` — Closer = 1. Visual range from group center. Opens Zone Out and pulses **MISSION BEGIN** / **SPAWN UNITS** (a timer, then the MCU).
- `Zone Out` — Closer = 0. Pulses **MISSION END** and **COOLDOWN**.
- `COOLDOWN` — then **Zone In ReActivate**. Time is 0 unless Spawn and **Allow multiple spawns** (then the minutes you set, default 5).
